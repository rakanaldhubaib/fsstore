...
const youtube = new YouTube("AIzaSyBInOkR0XO98Ccx5kWZAVFrnLrdd4k85q4"); //تعديل اساسي سوي اي بي اي جديد
const hastebins = require("hastebin-gen");
const getYoutubeID = require("get-youtube-id");
const yt_api_key = "AIzaSyBInOkR0XO98Ccx5kWZAVFrnLrdd4k85q4"; ///تعديل اساسي سوي اي بي اي جديد
...
client.on("ready", () => {
  ...
  client.user.setActivity(`𝐅𝐚𝐧 𝐒𝐲𝐬𝐭𝐞𝐦 𝐁𝐲 𝐍𝐨𝐭 𝐑𝐁𝐃 ✘`, { type: "WATCHING" });
});
...
client.on("message", message => {
  if (message.content === prefix + "bot") {
    const bot = new Discord.RichEmbed()
      ...
      .addField("**Bot Owner** :  ", `» <@592031131180728339>`, true) // تعديل اساسي غير الايدي لايدي حسابك
      .setImage("")
...
client.on("message", message => {
  if (message.content === "رابط") {
    message.channel.send(" https://discord.gg/THbqm4WDeF ");
    message.channel.sendFile("");
  }
});
...
client.on("message", message => {
  if (message.content.startsWith(prefix + "setby")) {
    ...
    client.on("guildMemberAdd", member => {
      ...
      var channel = member.guild.channels.find(gg => gg.name === "xg-log"); //// تعديل اساسي
      if (!channel) return;
